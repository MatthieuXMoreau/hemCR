...
pseudo.maturation.diff.filtered <- pseudo.maturation.diff %>% filter(qval < 1e-10)
```
...
## Plot CR specific trajectory

```{r}
# Extract Pallial neurons trajectory genes
CR.res <- as.data.frame(pseudo.maturation.diff.filtered[pseudo.maturation.diff.filtered$ABCs > 10,])
CR.genes <- row.names(CR.res)

CR_curve_matrix <- CR_curve_matrix[CR.genes, ]
```

### CR specific trajectory

```{r CR gene heatmap}
library(seriation)
# Order the rows using seriation
dst <- as.dist((1-cor(scale(t(CR_curve_matrix)), method = "spearman")))
row.ser <- seriation::seriate(dst, method ="GW_ward") #"R2E"
gene.order <- rownames(CR_curve_matrix[get_order(row.ser),])

#anno.colors <- list(lineage = c(Other_Neurons="#026c9a", CR_cells="#cc391b"),
...
pheatmap::pheatmap(Diff.curve_matrix[gene.order,
                   ...
                   #annotation_col = data.frame(lineage = rep(c("CR_cells","Other_Neurons"), each=200)),
...
# Order the rows using seriation
dst <- as.dist((1-cor(scale(t(CR_curve_matrix)), method = "spearman")))
row.ser <- seriation::seriate(dst, method ="GW_ward") #"R2E" #TSP #"GW"
gene.order <- seriation::get_order(row.ser)

anno.colors <- list(Gene.Clusters = c(Clust.1 ="#68b041",
                                      Clust.2="#e46B6b",
                                      Clust.3="#e7823a",
                                      Clust.4="#5ab793"#,
                                      #Clust.5="#68b041",
...
pheatmap::pheatmap(CR_curve_matrix[gene.order,], 
                   ...
                   cluster_rows = F,
                   cluster_cols = F,
                   #annotation_row = Gene.dynamique %>% dplyr::select(Gene.Clusters),
                   #annotation_colors = anno.colors,
                   show_colnames = F,
                   show_rownames = T,
                   fontsize_row = 2,
