...
```{r}
Neuro.trajectories <- FindVariableFeatures(Neuro.trajectories, selection.method = "disp", nfeatures = 2000, assay = "RNA")
```
...
pseudo.maturation.diff.filtered <- pseudo.maturation.diff %>% filter(qval < 1e-35)
```
...
```{r}
Signaling_And_Death_GOterm <- CR.gostres$result[CR.gostres$result$term_id %in% c("GO:0007267", "GO:0005102", "GO:0048018", "GO:0070997", "GO:0051402"),]

Signaling_And_Death_GOterm[,c(1,2,3,5,6,7,11)]
```
