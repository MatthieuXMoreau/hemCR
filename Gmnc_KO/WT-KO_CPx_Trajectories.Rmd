...
Plot.Genes.trend(Seurat.data= WT.KO.integrated,
                 group.by = "Genotype",
                 genes= c("Ttr","Htr2c", "Gmnc", "Trp73", "Foxj1", "Pifo", "Ccdc67"))
```
...
Diff.curve_matrix <- genSmoothCurves(gbm_cds[WT.CR.genes$Gene, ],
...
CPx.Gene.dynamique <- data.frame(Gene= names(Pseudotime.genes.clusters$clustering),
                                Waves= Pseudotime.genes.clusters$clustering,
                                Gene.Clusters = Pseudotime.genes.clusters$clustering) %>% arrange(Gene.Clusters)

row.names(CPx.Gene.dynamique) <- CPx.Gene.dynamique$Gene
CPx.Gene.dynamique$Gene.Clusters <- paste0("Clust.", CPx.Gene.dynamique$Gene.Clusters)
```
