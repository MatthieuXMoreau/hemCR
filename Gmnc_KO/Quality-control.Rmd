...
```{r}
#Export metadata
Low.detected.Genes <- c("Scrublet", as.character(Raw.data$nFeature_RNA < 10^3.2))
Low.detected.Genes <- paste(Low.detected.Genes, sep=",", collapse=",")

Scrublet <- c("Scrublet", Raw.data$Predicted_doublets)
Scrublet <- paste(Scrublet, sep=",", collapse=",")

Cellgrouping <- rbind(Low.detected.Genes, Scrublet)
