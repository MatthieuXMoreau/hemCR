...
CR_genes <- read.table("../CajalRetzius_trajectory/CR_dynamic_genes.csv", sep = ";", header = T)[,1]
Input.TFs <- intersect(rownames(gbm_cds_KO)[rownames(gbm_cds_KO) %in% CR_genes], rownames(gbm_cds_WT)[rownames(gbm_cds_WT) %in% CR_genes])
...
p6 <- pheatmap::pheatmap(Smoothed.point.cor,
         ...
         show_rownames = F,
         color = colorRampPalette(brewer.pal(n = 10, name = "RdBu"))(100),
         main = "CP WT vs CP WT euclidian dist",
         legend = F,
         annotation_legend =F)
```
