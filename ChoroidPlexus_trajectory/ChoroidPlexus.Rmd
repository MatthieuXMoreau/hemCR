...
```{r}
Prog.data <-  subset(ChP.data, idents = c("ChP_progenitors", "ChP"))
...
```{r}
Prog.data <- subset(Prog.data,
                    subset = Barcodes %in% rownames(CellCycledata))
...
p4 <-FeaturePlot(object = Prog.data,
            features = c("Gmnc"),
            pt.size = 1,
            cols = c("grey90", brewer.pal(9,"YlGnBu")),
            reduction = "spring",
...
FeaturePlot(object = Prog.data,
            features = c("Pseudotime", "Revelio.cc"),
            pt.size = 2,
            cols = rev(colorRampPalette(brewer.pal(n =10, name = "Spectral"))(100)),
            reduction = "spring",
            order = T) & NoAxes()
...
```{r}
# Transfer metadata
meta.data <- data.frame(Barcode= Prog.data$Barcodes,
                        Pseudotime= Prog.data$Pseudotime,
                        Cell.cycle= Prog.data$Revelio.cc)

Annot.data  <- new('AnnotatedDataFrame', data = meta.data)

# Transfer counts data
Prog.data <- FindVariableFeatures(Prog.data, selection.method = "vst", nfeatures = 2000)
var.genes <- VariableFeatures(Prog.data)
count.data = data.frame(gene_short_name = rownames(Prog.data[["RNA"]]@data[var.genes,]),
                        row.names = rownames(Prog.data[["RNA"]]@data[var.genes,]))

feature.data <- new('AnnotatedDataFrame', data = count.data)

# Create the CellDataSet object including variable genes only
gbm_cds <- newCellDataSet(Prog.data[["RNA"]]@counts[var.genes,],
...
```{r}
rm(list = ls()[!ls() %in% c("Prog.data", "gbm_cds")])
gc()
```
...
```{r cache=TRUE}
pseudo.maturation.diff <- differentialGeneTest(gbm_cds[fData(gbm_cds)$num_cells_expressed >= 10,], 
...
Pseudotime.genes.clusters <- cluster::pam(as.dist((1 - cor(Matrix::t(Smooth.curve.matrix),method = "pearson"))), k= 5)
...
dst <- as.dist((1-cor(scale(t(Smooth.curve.matrix)), method = "pearson")))
