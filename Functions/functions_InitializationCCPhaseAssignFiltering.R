...
getCellCyclePhaseAssign_allcells <- function (dataList) 
{
  startTime <- Sys.time()
  cat(paste(Sys.time(), ": assigning cell cycle phases: ", 
            sep = ""))
  ...
  colnames(phaseScoreNormalized) <- paste(colnames(phaseScoreNormalized), 
                                          "_zScore", sep = "")
  phaseAssignStatistics <- data.frame(ccPhase = resultList[["ccPhase"]], 
                                      meanOfPhaseScore = apply(resultList[["phaseScore"]], 
                                                               1, mean), sdOfPhaseScore = apply(resultList[["phaseScore"]], 
                                                                                                1, sd), highestPhaseScore = resultList[["highestPhaseScore"]], 
                                      secondHighestPhaseScore = resultList[["secondHighestPhaseScore"]])
  phaseAssignStatistics <- cbind(phaseAssignStatistics, phaseScore, 
                                 phaseScoreNormalized)
  ...
  phaseDifferenceBetweenHighestAndSecondHighest <- t(subset(phaseAssignStatistics, 
                                                            select = colnames(phaseScoreNormalized)))
  highestPhaseScoreIndex <- apply(phaseDifferenceBetweenHighestAndSecondHighest, 
                                  2, which.max)
  highestPhaseScoreIndexOfMatrix <- (1:length(highestPhaseScoreIndex)) * 
    dim(phaseDifferenceBetweenHighestAndSecondHighest)[1] - 
    dim(phaseDifferenceBetweenHighestAndSecondHighest)[1] + 
    highestPhaseScoreIndex
  phaseDifferenceBetweenHighestAndSecondHighest[highestPhaseScoreIndexOfMatrix] <- -5
  secondHighestPhaseScoreIndex <- apply(phaseDifferenceBetweenHighestAndSecondHighest, 
                                        2, which.max)
  differenceHighestIndexToSecondIndex <- apply(cbind(abs(highestPhaseScoreIndex - 
                                                           secondHighestPhaseScoreIndex), abs(highestPhaseScoreIndex + 
                                                                                                ...
                                                                                                secondHighestPhaseScoreIndex), abs(highestPhaseScoreIndex - 
                                                                                                                                     dim(phaseDifferenceBetweenHighestAndSecondHighest)[1] - 
                                                                                                                                     secondHighestPhaseScoreIndex)), 1, min)
  isOutlierSuspectedDoublets <- as.logical(rep(FALSE, dim(phaseAssignStatistics)[1]))
  isOutlierNoConfidenceInPhaseScore <- as.logical(rep(FALSE, 
                                                      dim(phaseAssignStatistics)[1]))
  isOutlierSuspectedDoublets[which((differenceHighestIndexToSecondIndex > 
                                      dataList@datasetInfo$ccPhaseAssignMaxDiffOfCCPhasesBetweenHighestScores) & 
                                     (phaseAssignStatistics[, "secondHighestPhaseScore"] > 
                                        dataList@datasetInfo$ccPhaseAssignThresholdSecondHighestPhaseScore))] <- TRUE
  isOutlierNoConfidenceInPhaseScore[which(phaseAssignStatistics[, 
                                                                "highestPhaseScore"] < dataList@datasetInfo$ccPhaseAssignThresholdHighestPhaseScore)] <- TRUE
  phaseAssignStatistics <- cbind(phaseAssignStatistics, isOutlierSuspectedDoublets, 
                                 isOutlierNoConfidenceInPhaseScore)
  ...
  # Prevent cell filtering so all cells are returned with cell cycle phase assignment
